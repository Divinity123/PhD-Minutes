...
for keyword in keyword_set:
    ...
    for document in padded_documents:
        if keyword in document[1]:
            index[keyword].append(document[0])


# ----------------------------------------------------------------
# output the co-occurrence counts for attack
directory = './output/test'
fp = open(directory, 'w+')
for ii in range(len(keyword_list)-1):
    fp.write(keyword_list[ii] + ',')
fp.write(keyword_list[-1] + '\n')

for keyword1 in keyword_list:
    set1 = set(index[keyword1])
    for ii in range(len(keyword_list)-1):
        set2 = set(index[keyword_list[ii]])
        intersection = set1 & set2
        fp.write(str(len(intersection)) + ',')
    set2 = set(index[keyword_list[-1]])
    intersection = set1 & set2
    fp.write(str(len(intersection)) + '\n')
fp.close()
