...
\begin{definition}[Information Channel]
	A channel is a triple $(\mathcal{X}, \mathcal{Y}, \mathcal{C})$, where $\mathcal{X}$ and $\mathcal{Y}$ are finite sets and $\mathcal{C}$ is a channel matrix, an $|\mathcal{X}| \times |\mathcal{Y}|$ matrix whose entries are between 0 and 1 and whose rows each sum to 1. 
\end{definition}
...
\begin{equation}
	V(\pi, \mathcal{C}) = \sum_{y \in \mathcal{Y}} \max_{x in \mathcal{X}} \pi [x] \mathcal{C} [x,y].
\end{equation}
\end{definition}


\begin{definition}[Leakage and Capacity]
We define \textit{min-entropy leakage} $\mathcal{L}(\pi, \mathcal{C})$ and \textit{min-capacity} $\mathcal{ML}(\mathcal{C})$ to be:
\begin{align}
\mathcal{L}(\pi, \mathcal{C}) &= - \log V(\pi) + \log V(\pi, \mathcal{C}) = \log \frac{V(\pi, \mathcal{C})}{V(\pi)} \\
\mathcal{ML}(\mathcal{C}) &= \sup_{\pi} \mathcal{L}(\pi, \mathcal{C}).  
\end{align}
\end{definition}
...
\textit{G-leakage} $\mathcal{L}_{g} (\pi, \mathcal{C})$ and \textit{g-capacity} $\mathcal{ML}_{g} (\mathcal{C})$ are defined as:
\begin{align*}
\mathcal{L}_{g} (\pi, \mathcal{C}) & = - \log V_{g}(\pi) + \log V_{g}(\pi, \mathcal{C}) = \log \frac{V_{g}(\pi, \mathcal{C})}{V_{g}(\pi)} \\
\mathcal{ML}_{g}(\mathcal{C}) &= \sup_{\pi} \mathcal{L}_{g}(\pi, \mathcal{C}). 
\end{align*} 
\end{definition}
