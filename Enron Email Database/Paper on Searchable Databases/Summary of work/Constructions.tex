...
Co-occurrence at level $l$ on database $\db$ with documents is defined to be $\dcm_{l}^{\db}: \kwset(\db)^l \rightarrow \doc^{*}$.
\begin{equation}
	\dcm_{l}^{\db}(\kw_1, \dots, \kw_l) := \{ \doc \mid  (\kw_i \neq \kw_j \forall i,j) \wedge (\doc \in \db) \wedge (\{\kw_1, \dots, \kw_l\} = \kwset(\doc)) \}.
\end{equation}
\end{definition}
...
\begin{definition}[Co-occurrence at level $d$ with Encrypted Documents]
	Co-occurrence at level $l$ on encrypted database $\edb$ with encrypted documents is defined to be $\edcm_{l}^{\edb}: \ekwset(\edb)^l \rightarrow \edoc^{*}$.
	\begin{equation}
	\edcm_{l}^{\edb}(\ekw_1, \dots, \ekw_l) := \{ \edoc \mid  (\ekw_i \neq \ekw_j \forall i,j) \wedge (\edoc \in \edb) \wedge (\{\ekw_1, \dots, \ekw_l\} = \dec(\edoc)) \}.
	\end{equation}
\end{definition}
...
\procedure{$\enc(\secparam, (\key, \perm_a), \db)$}{%
	\pcln \db' = () \\
	\pcln \pcfor \doc \in \db: \\
	\pcln \pcind[1] \db' = \db' + (\kwset(\doc), (True, \file(\doc))) \\
	\pcln \pcind[1] \pcfor i = 1, \dots, a-1: \\
	\pcln \pcind[2] \db' = \db' + ((P_a)^i(\kwset(\doc)), (False, \texttt{FGen}(\secparam, (P_a)^i(\kwset(\doc)))) \\
...
\procedure{$\dec(\secparam, (\key, \perm_a), \edb)$}{%
	\pcln \db = () \\
	\pcln \pcfor \edoc \in \edb: \\
	\pcln \pcind[1] (\kwset, (b, \file)) \gets \overline{\dec}(\secparam, (\key, \perm_a), \edoc) \\
...
\procedure{$\enc(\secparam, (\key, \perm_a), \db)$}{%
	\pcln \db' = () \\
	\pcln \pcfor \doc \in \db: \\
	\pcln \kwset' = \cup_{i=0}^{a-1} (P_a)^i(\kwset(\doc))\\
	\pcln \pcind[1] \db' = \db' + (\kwset', \file(\doc)) \\
	\pcln \pcreturn \overline{\enc}(\secparam, \key, \db')
}
...
\procedure{$\enc(\secparam, (\key, \perm_a), \db)$}{%
	\pcln \db' = () \\
	\pcln \pcfor \{\doc_1, \cdots, \doc_a\} \sample \db: \\
	\pcln \pcind[1] \kwset' = \cup_{i=1}^{a} (P_a)^{a-i+1} (\kwset(\doc_i)) \\
	\pcln \pcind[1] db' = db' + ((\kwset', \file(\doc_1))) \\
	\pcln \pcind[1] \pcfor j = 2, \cdots, a: \\
	\pcln \pcind[2] \kwset' = (P_a) (\kwset') \\
	\pcln \pcind[2] \db' = \db' + ((\kwset', \file(\doc_j))) \\
	\pcln \pcind[1] \db = \db \backslash \{\doc_1, \cdots, \doc_a\} \\
	\pcln \pcreturn \overline{\enc}(\secparam, \key, \db')
}
\end{pchstack}
