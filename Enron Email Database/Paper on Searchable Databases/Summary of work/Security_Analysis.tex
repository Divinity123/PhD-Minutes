...
\subsection{All of our Constructions are Secure against Inference Attack}
In this subsection, we show that all of our schemes are secure against inference attack. We will use extended posterior g-vulnerability to analyse the schemes. For simplicity, we assume that the database is a point distribution, i.e. the adversary knows exactly how the database looks like. The gain function we consider here is the following:
\begin{itemize}
	\item $\mathcal{W} = \kwset(\db) \times \ekwset(\edb)$,
	\item $\mathcal{X} = \dcm \times \mathcal{P}$,
	\item $\mathcal{Y} = \leak(\mathcal{X}) = \{\enc(\dcm, \sigma) \mid \sigma \in \mathcal{P} \}$,
...
\begin{theorem}
Let the distribution of database be a fixed distribution $\pi$, i.e. the adversary knows that the unencrypted database is $\db$. Let $\mathcal{X}$ be the secrets and $\mathcal{Y}$ be the leakage. given $y \in \mathcal{Y}$, and $\leak$ the leakage function for the encryption scheme. We assume that there is a unique $x \in \mathcal{X}$ such that $\leak(x) = y$. Then, for the gain function $g$ defined above,
\begin{equation}
	EV_g(\pi, \mathcal{C}_{\leak}) = 1
\end{equation}
\end{theorem}
...
= & \sum_{y \in {\mathcal{Y}}} \prob{\mathcal{Y} = y} \max_{w \in {\mathcal{W}}} \prob{\mathcal{X} = \leak^{-1}(y) \mid \mathcal{Y} = y} g(w, \leak^{-1}(y), y) \\
= & \sum_{y \in {\mathcal{Y}}} \prob{\mathcal{Y} = y} \max_{w \in {\mathcal{W}}} 1 \cdot g(w, \leak^{-1}(y), y) \\
= & \sum_{y \in {\mathcal{Y}}} \prob{\mathcal{Y} = y} \cdot 1 \\
= & 1.
\end{align*}
The proof uses the property that $\prob{\mathcal{X} = x \mid \mathcal{Y} = y}$ is 1 when $x = \leak^{-1}(y)$ and 0 otherwise. Furthermore, since the adversary knows the pre-image completely, his maximal guess is 1 for every $g(\cdot)$.
\end{proof}
...
\begin{equation}
	EV_g(\pi, \mathcal{C}_{\leak}) = \frac{1}{a}.
\end{equation}
\end{proposition}
...
\begin{theorem}
Let the leakage channel and gain function be those described above. Furthermore, we assume that for all $y \in \mathcal{Y}, (\perm_a)(y) \neq y$. Then
\begin{equation}
	EV_g^{\texttt{scheme1}}(\pi, \mathcal{C}_{\leak}) = \frac{1}{a}.
\end{equation}
\end{theorem}

\begin{proof}
We observe that for any leakage $y = \ekwset$, if we know the encryption function is $Q \cdot (\perm_a)^i$ for some $i$, then the set of keywords for the unencrypted document is exactly $(Q \cdot (\perm_a)^i)^{-1}(\ekwset)$. Because $(\perm_a)(y) \neq y$ for all $y \in \mathcal{Y}$, $(Q \cdot (\perm_a)^i)^{-1}(\ekwset)$ is the exact set of keywords only when $(Q \cdot (\perm_a)^i)^{-1}$ is the correct decryption function. This happens with probability $\frac{1}{a}$. In terms of extended posterior g-vulnerability, we have
\begin{align*}
  &EV_g^{\texttt{scheme1}}(\pi, \mathcal{C}_{\leak})\\
= &\sum_{y \in {\mathcal{Y}}} \prob{\mathcal{Y} = y} \max_{w \in {\mathcal{W}}} \sum_{x \in {\mathcal{X}}} \prob{\mathcal{X} = x \mid \mathcal{Y} = y} g(w, x, y)\\
...
\textbf{\texttt{Scheme2} and \texttt{scheme3} are more Secure.} Now we show that \texttt{Scheme2} and \texttt{scheme3} achieves better security with respect to the gain function. We will do this by comparing the extended posterior g-vulnerability of the three schemes without explicitly computing them. Hence, demonstrating the advantage of using extended posterior g-vulnerability to evaluate security of encryption schemes.


\begin{theorem}
Let the leakage channel and gain function be those described above. Furthermore, we assume that for all $y \in \mathcal{Y}, (\perm_a)(y) \neq y$. Then
\begin{equation}
EV_g^{\texttt{scheme1}}(\pi, \mathcal{C}_{\leak}) \geq EV_g^{\texttt{scheme2}}(\pi, \mathcal{C}_{\leak}) \geq EV_g^{\texttt{scheme3}}(\pi, \mathcal{C}_{\leak}).
\end{equation}
\end{theorem}


\begin{proof}
We examine the sets of keywords which can possibly give leakage $y$ in each scheme, and compare the conditional probability $\prob{x \mid y}$ for the schemes. For \texttt{scheme1}, we see that $\{((Q \cdot (\perm_a)^i)^{-1}(y), Q \cdot (\perm_a)^i) \mid i = 0, \dots, a-1\}$ is the set of $x$ that can generate leakage $y$. Each occurs with probability $\frac{1}{a}$ and only one of them contains the correct keyword set. So $\max_{w \in {\mathcal{W}}} \sum_{x \in {\mathcal{X}}} \prob{\mathcal{X} = x \mid \mathcal{Y} = y} g(w, x, y) = \frac{1}{a}$.

On the other hand, the feasible keyword sets is $\bar{\mathcal{X}} = \{ (\perm_a)^{i}(x) \mid (x \subset y) \wedge (i = 0, \dots, a-1) \wedge (\cup_{i=0}^{a-1} (\perm_a)^{i}(x) = y) \}$. Each keyword set can be paired with any encryption function $Q \cdot (\perm_a)^j$ for $j = 0, \dots, a-1$. Therefore, we only need to look at the distribution of elements in $\bar{\mathcal{X}}$. But there are at least $a$ elements (since $y$ is not $\perm_a$-permutable), so the probability of guessing the correct keyword is less than or equal to $\frac{1}{a}$. This means $\max_{w \in {\mathcal{W}}} \sum_{x \in {\mathcal{X}}} \text{Pr}_{\texttt{scheme2}}[\mathcal{X} = x \mid \mathcal{Y} = y] g(w, x, y) \leq \max_{w \in {\mathcal{W}}} \sum_{x \in {\mathcal{X}}} \text{Pr}_{\texttt{scheme1}}[\mathcal{X} = x \mid \mathcal{Y} = y] g(w, x, y)$. Thus, $EV_g^{\texttt{scheme1}}(\pi, \mathcal{C}_{\leak}) \geq EV_g^{\texttt{scheme2}}(\pi, \mathcal{C}_{\leak})$.


For \texttt{scheme3}, we see that the set of feasible keyword sets contains all of $\bar{\mathcal{X}}$. At the same time, we have that each keyword set can be paired with any encryption function $Q \cdot (\perm_a)^j$ for $j = 0, \dots, a-1$. So it must be the case that $\max_{w \in {\mathcal{W}}} \sum_{x \in {\mathcal{X}}} \text{Pr}_{\texttt{scheme3}}[\mathcal{X} = x \mid \mathcal{Y} = y] g(w, x, y) \leq \max_{w \in {\mathcal{W}}} \sum_{x \in {\mathcal{X}}} \text{Pr}_{\texttt{scheme2}}[\mathcal{X} = x \mid \mathcal{Y} = y] g(w, x, y)$. Therefore, $EV_g^{\texttt{scheme2}}(\pi, \mathcal{C}_{\leak}) \geq EV_g^{\texttt{scheme3}}(\pi, \mathcal{C}_{\leak})$.
\end{proof}
