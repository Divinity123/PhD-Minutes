...
\begin{itemize}
	\item Data used is the PUDF base data from first quarter of 2010. This data set is publicly available unlike NIS data set. Since we only need to demonstrate feasibility of the attack, it suffices to use the earlier data set if we can find some correlated columns.
...
\begin{equation*}
	\text{Bimon}\left(\frac{N(i,j)}{S(i,j)}, \frac{1}{S(i,j)}\right).
\end{equation*}
